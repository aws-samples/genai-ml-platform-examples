...
def is_onnx_mode() -> bool:
    text = CONFIG_PATH.read_text()
    return 'key: "use_onnx"' in text and '"true"' in text

def is_orig_decoder_joint() -> bool:
    text = CONFIG_PATH.read_text()
    return 'key: "use_orig_decoder_joint"' in text and '"true"' in text

def warmup_trt_cache():
    """Build TRT engine cache once before Triton spawns multiple instances."""
    sys.path.insert(0, "/models/parakeet_asr/1")
    if is_orig_decoder_joint():
        from asr_model_onnx import ASRModelHybrid
        ASRModelHybrid.warmup_trt_cache(ONNX_DIR)
    else:
        from asr_model_onnx import ASRModelOnnx
        ASRModelOnnx.warmup_trt_cache(ONNX_DIR)
...
def main():
    ...
    # Build TRT engine cache before Triton starts, so all instances reuse it
    if is_onnx_mode() and Path(ONNX_DIR).exists():
        warmup_trt_cache()
