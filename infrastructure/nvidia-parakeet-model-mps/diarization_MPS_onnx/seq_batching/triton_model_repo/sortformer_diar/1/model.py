...
CONFIG_FILENAME = "sortformer_streaming_config.json"
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        # Load streaming config from co-located JSON
        config_path = os.path.join(os.path.dirname(__file__), CONFIG_FILENAME)
        with open(config_path) as f:
            streaming_cfg = json.load(f)

        self.collar_sec = streaming_cfg.get("default_collar_sec", 0.25)
        self.min_speech_prob = streaming_cfg.get("default_min_speech_prob", 0.3)
        self.online_seg_mode = streaming_cfg.get("online_segmentation_mode", "argmax")
        self.online_tail_sec = streaming_cfg.get("online_tail_sec", 1.5)
        self.sortformer_modules_cfg = streaming_cfg.get("sortformer_modules_config", {})
        ...
        self.use_onnx = False
        if "use_onnx" in params:
            self.use_onnx = params["use_onnx"]["string_value"].lower() in ("true", "1", "yes")

        if self.use_onnx:
            from .sortformer_onnx import SortformerEncLabelModelOnnx  ## lazy load only if needed
            self.model = SortformerEncLabelModelOnnx(model_name, device=self.device)
        else:
        ...
        self.online_tail_frames = max(1, int(round(self.online_tail_sec / self.frame_duration_sec)))
    ...
    def _configure_streaming(self):
        m = self.model
        cfg = self.sortformer_modules_cfg
        m.streaming_mode = True
        m.sortformer_modules.chunk_len = cfg.get("chunk_len", 120)
        m.sortformer_modules.chunk_right_context = cfg.get("chunk_right_context", 1)
        m.sortformer_modules.fifo_len = cfg.get("fifo_len", 60)
        m.sortformer_modules.spkcache_update_period = cfg.get("spkcache_update_period", 124)
        m.sortformer_modules.spkcache_len = cfg.get("spkcache_len", 254)
    ...
    def _current_segments_cpu(self, preds_cpu: torch.Tensor, committed: list[str], emitted: int) -> list[str]:
        ...
        tail_lines = self._preds_to_segments(
            preds_cpu, self.online_seg_mode,
    ...
    def _finalize_segments_cpu(self, preds_cpu: torch.Tensor, committed: list[str], emitted: int, mode: str) -> list[str]:
        ...
        return self._format_segments(
            self._merge_segment_lists(committed, tail_lines), self.collar_sec,
        )
    ...
    def _advance_committed_segments_cpu(self, tail_preds_cpu: torch.Tensor, state: SequenceState, emitted: int, total_frames: int) -> None:
        ...
        new_lines = self._preds_to_segments(
            new_preds, self.online_seg_mode, offset_sec=offset_sec, apply_collar=False,
        )
    ...
    def _preds_to_segments(self, preds, mode, offset_sec=0.0, apply_collar=True) -> list[str]:
        ...
        return self._format_segments(lines, self.collar_sec if apply_collar else 0.0)
    ...
    def _argmax_segments(self, preds: torch.Tensor, offset_sec: float = 0.0) -> list[str]:
        ...
        mask = max_probs.numpy() < self.min_speech_prob
        labels[mask] = -1
